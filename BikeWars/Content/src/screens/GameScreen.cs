...
namespace BikeWars.Content.screens
{
    public class GameScreen : IScreen
    {
        ...
        private Dictionary<Achievement, float> _showAchievementTimes;
        ...
        public GameScreen(AudioService audioService, GameMode gameMode, bool isTechDemo = false)
        {
            ...
            _showAchievementTimes = new Dictionary<Achievement, float>();
        ...
        public virtual void LoadContent(ContentManager content, GraphicsDevice gd)
        {
            ...
            _achievementsManager.Succeeded += AddAchievementToShow;
            ...
            foreach (var p in state.Projectiles)
            {
                switch(p.Basic.Type)
                {
                    ...
                    case SaveLoad.TYPES.THROWBEER:
                    {
                        ...
                        if (_gameObjectManager.Player1 != null)
                        {
                            ThrowBeer tb = new ThrowBeer(p.Basic.Position.ToVector2(),
                                _gameObjectManager.Player1.Transform.Position,
                                _gameObjectManager.Player1); // TODO player doesn't make sense here
                            tb.HasHit = p.HasHit;
            ...
            foreach (var o in state.Objects)
            {
                ...
                switch(o.Type)
                {
                    ...
                    case SaveLoad.TYPES.DESTRUCTIBLE:
                        if (o.Health == null || o.SpriteKey == null)
                            break;
        ...
        public void Unload()
        {
            ...
            if (_collisionManager != null)
            {
                ...
                if (p1 != null)
                {
                    _collisionManager.OnItemPickup -= p1.OnPickUpItem;
                    p1.ItemPickedUp -= _collisionManager.OnRemoveItem;
                }
                _collisionManager.OnTramHit -= _combatManager.HandleTramHit;
            ...
            var gm = _gameObjectManager;
            if (gm != null)
            {
                var p1 = gm.Player1;
                if (p1 != null)
                {
                    // p1.OnLevelUp -= _onPlayerLevelUp;
                    p1.OnBikeShopOpen -= _onBikeShopOpen;
                }
            }
            _bikeShopScreen.Closed -= _onBikeShopClose;
        ...
        {
            List<Achievement> toRemove = new();
            List<Achievement> achievements = _showAchievementTimes.Keys.ToList();
            float delta = (float)gameTime.ElapsedGameTime.TotalSeconds;
            int index = 1;
            foreach (Achievement ach in achievements)
            {
                _showAchievementTimes[ach] -= delta;
                if (_showAchievementTimes[ach] <= 0)
                {
                    toRemove.Add(ach);
                    continue;
                }
                AchievementsComponent ac = new AchievementsComponent(ach);
                ac.Draw(sb, RenderPrimitives.Pixel, Color.White, new Vector2(ViewPort.Width - AchievementsComponent.WIDTH_OF_COMPONENT, ViewPort.Height - index * AchievementsComponent.HEIGHT_OF_COMPONENT), UIAssets.DefaultFont);
                index += 1;
            }
             foreach (Achievement a in toRemove)
            {
                _showAchievementTimes.Remove(a);
            }
        }
